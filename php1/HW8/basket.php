...
JOIN orders ON orders.id = orders_products.order_id WHERE orders.status = 'active' AND orders.user_id = {$_SESSION['id']};");
    ...
    $basketData = mysqli_query(getDb(),"SELECT orders_products.order_id, orders_products.product_id, orders_products.total AS quantity, products.name, products.price, (products.price*orders_products.total) AS totalPrice, SUM(orders_products.total * products.price) OVER(PARTITION BY orders_products.session_id) AS grandTotal, product_images.title AS imageName FROM orders_products
JOIN products ON products.id = orders_products.product_id
